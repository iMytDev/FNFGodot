...
@tool
class_name FunkinText extends Node2D
const AnimatedLetter = preload("uid://bkden22qtubfn")
...
func set_text(newText: String):
	...
	for letters in letters_in_lines: for i in letters: if i: remove_child(i)
...
func update_letters_position(h_aligment: HorizontalAlignment = horizontal_alignment):
	...
	for i in letters_in_lines:
		...
		for letter: AnimatedLetter in i: 
			if !letter: 
				...
				continue
			letter.position = Vector2(curTextPos,offset_y)
	...
	match h_aligment:
		HORIZONTAL_ALIGNMENT_CENTER: 
			...
			for letters in letters_in_lines:
				...
				for i in letters: if i: i.position.x -= center
				_line += 1
	...
	match vertical_alignment:
		VERTICAL_ALIGNMENT_CENTER:
			...
			for l in letters_in_lines: for i in l: if i: i.position.y -= center
			
		VERTICAL_ALIGNMENT_BOTTOM:
			for l in letters_in_lines: for i in l: if i: i.position.y -= height
			


func set_x(newX): x = newX; position.x = x
func set_y(newY): y = newY; position.y = y
