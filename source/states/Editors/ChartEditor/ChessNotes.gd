...
var _beat_lines: Array[SolidNode2D]
func _ready() -> void: super(); add_child(notes); _create_beat_lines()

func _draw() -> void: super(); _create_beat_lines()


func _update_beat_lines_size() -> void:
	var index: int = _beat_lines.size()
	while index: index -= 1; _beat_lines[index].size = Vector2(size.x,3)
